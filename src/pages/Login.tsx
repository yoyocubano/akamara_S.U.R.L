...
const Login = () => {
    ...
    const startMagicTimer = () => {
        console.log("⏳ Iniciando secuencia de seguridad...");
        setIsPressing(true);
    ...
    const cancelMagicTimer = () => {
        ...
        if (isPressing) {
            console.log("❌ Secuencia cancelada (soltaste antes de tiempo).");
            setIsPressing(false);
        }
    };
    ...
    const performMagicLogin = async () => {
        setLoading(true);
        const magicEmail = 'admin@akamara.cu';
        const magicPass = 'akamara2026';
        
        try {
            await account.createEmailPasswordSession(magicEmail, magicPass);
            navigate('/admin');
        } catch (err: any) {
            console.error("Magic Login Failed:", err);
            setError("Fallo en Acceso de Emergencia");
            setLoading(false);
        } finally {
            setIsPressing(false);
        }
